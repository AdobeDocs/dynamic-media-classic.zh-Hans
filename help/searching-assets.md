...
uuid: 058209bc-bac4-4d5 c-8261-e242 a543 beffa
contentOwner: admin
content-type: 引用
products: SG_ EXPERIENCE MANAGER/Dynamic-Media-Scene-7
geptopics: SG_ SCENESELEFERENDER_ PK/categories/managing_ assets
discoiquuid: efef4e7-37c5-42e2-9266-ed026 design628
...
要在 Scene7 Publishing System 中查找文件资源，可以按类型查看资源、排序浏览面板中的资源、执行简单搜索、执行高级搜索并对资源进行过滤。
...
>[!NOTE]
>
>动态媒体经典可跟踪搜索。要再次运行搜索，请选择“搜索”按钮并从“搜索”菜单底部选择该搜索的名称。
...
**按资产类型过滤只需在菜单上选择一个资产类型，即可** 将搜索范围缩小到一个资产类型。

**“文件和文件夹** ”选择要搜索的位置：在“所有文件和文件夹”中、“选定的文件夹”中或“在选定的文件夹和子文件夹内”。

**所有“发布状态** ”搜索已准备好发布的文件，未标记为可发布或所有文件。

**条件** 如果您指定用于搜索的元数据条件，请选择搜索是否必须匹配所有条件(一个全部搜索)或任何条件(一个OR搜索)。
...
1. 单击面板上的“字段”按钮 ，执行以下指令显示过滤菜单并创建过滤器操作：
   ...
   **选择元数据字段** 选择过滤菜单上的字段名称。

   ***注意**：只有默认元数据视图中的元数据字段名称显示在过滤菜单上。*

   请参阅[元数据视图](application-setup.md#metadata_views)。

   **添加元数据字段** 选择添加面板。在“过滤器”窗格显示出面板后，单击其“字段”按钮，并在过滤菜单上选择元数据字段的名称。

   **删除元数据字段** 在筛选菜单上选择删除此面板。
...
**保存过滤器操作** 单击选择预设按钮，选择“将当前预设保存为新预设”，然后在“保存”对话框中输入名称。

**重复过滤操作** 单击选择预设按钮，并在菜单上选择过滤器操作的名称。菜单将列出您所保存的过滤器操作。

**从“选择预设”菜单** 中删除过滤器操作，运行筛选器操作。然后单击“选择预设”按钮，并选择菜单上的“删除预设”。
...
**即时发布** 自动发布任何元数据更改(包括新资产、关键字更改等)。

**XMP包** 刷新XMP包。该数据包不能用于搜索，但可提供最新的 XMP。

**关键字** 将您的关键字发布到元数据服务器以在搜索中使用。

...
对于模板，选择 **[!UICONTROL 添加和预览]**，然后设置您选择的参数。 其他选件集类型不包含参数，但您仍可以通过选择 **[!UICONTROL 预览]** 和更改可用的预设。
...
* **图像模板**  — 您可以在Adobe Dynamic Media Classic中使用 **[!UICONTROL 生成]** >模板基础知识命令。 通过参数，可换出及自定义模板的各组成部分（文本框架中的文本、不同的图像）。对于优惠套餐，举例来说，您可以使用模板参数在优惠套餐中的同一图像上创建变型。有关创建图像模板和将图像模板参数化的信息，请参阅 [创建模板参数](creating-template-parameters.md#creating_template_parameters).
...
1. 完成后，在页面的右下角附近，请确保 **[!UICONTROL 保存后发布*]** （默认）。
...
1. （可选）选择图像或视频，然后选择 **[!UICONTROL 预览]**. 在“预览选件”页面上，您可以更改所选图像或视频的大小和外观。 或者，您也可以更改选件集中的所有图像或视频。
   ...
   * 选择预设以更改图像或视频的外观和大小。
   * 要将您选择的预设应用于选件集中的所有选件，请选择 **[!UICONTROL 选择全部预设]** 复选框。

   选择 **[!UICONTROL 保存]** 以保存对图像或视频选件所做的更改。 然后选择 **[!UICONTROL 关闭]** 返回Test&amp;Target选件集页面。

1. 完成为选件集创建选件并为不同图像选择图像预设后，请确保 **[!UICONTROL 保存后发布]** （默认）。
...
1. 要编辑选件集，请在网格视图或列表视图中显示选件集，然后选择其 **[!UICONTROL 编辑]** “滚动”按钮。
1. 在Test&amp;Target选件集页面中，执行以下任一操作：

   * **删除选件**  — 选择选件，然后选择 **[!UICONTROL 删除]** 从集中删除选件。
   * **添加选件**  — 如何添加选件取决于您正在使用的选件集类型：
      * **模板**  — 选择 **[!UICONTROL 添加和预览]**，然后在“添加和预览选件”页面上，创建其他选件。
...
1. 完成编辑时，在页面的右下角附近，确保 **[!UICONTROL 保存后发布]** （默认）。

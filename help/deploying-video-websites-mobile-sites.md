...
网站、移动设备网站和桌面应用程序可使用URL字符串或嵌入代码访问Dynamic Media Classic服务器内容（包括视频）。 Dynamic Media Classic在发布过程中激活这些URL字符串。 要将视频的URL字符串或嵌入代码放入网页、移动页面和桌面应用程序中，请从Dynamic Media Classic中复制该代码。
...
发布视频后，Dynamic Media Classic服务器可以将视频交付到您的网站、移动设备网站或应用程序。
...
Dynamic Media Classic为编码视频和预编码视频生成缩略图。 您可以像使用任何图像资源一样使用视频缩略图。此外，您还可以获取Dynamic Media Classic生成的视频缩略图的URL，并在Dynamic Media Classic之外部署这些URL。 例如，您可以在搜索结果、相关视频列表和网站上的视频播放列表中部署缩略图。
...
请参阅[Image Serving指南](https://experienceleague.corp.adobe.com/docs/dynamic-media-developer-resources/image-serving-api/image-serving-api/c-is-home.html?lang=en#image-serving-api)。

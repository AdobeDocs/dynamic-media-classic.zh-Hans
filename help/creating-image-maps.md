...
1. 执行以下任一操作：
   ...
   * 如果您在网格视图或列表视图中处理图像，请在编辑下拉列表中选择 **[!UICONTROL 图像映射]**. 或者，在“详细信息视图”中打开它，然后选择 **[!UICONTROL 图像映射]** 图像上方。
...
1. 绘制矩形或多边形（多条边）图像映射：
   ...
   * **矩形图**  — 选择“矩形图像映射”工具并在页面上拖动以创建矩形。 要向矩形映射添加点（从而将其更改为多边形映射），请按Ctrl，然后将插入工具放置到所需位置并选择。

   * **多边形地图**  — 选择多边形图像映射工具，并选择要包围的图像区域周长上的点。 使用多边形密度滑块改变多边形中的点密度。如果选择其他映射，将记住原始密度。如果在多边形中添加、删除或移动任何点，将丢失原始密度，同时滑块重置为其最大值。

1. 如果需要，在“图像映射”列表中为图像映射输入名称。绘制图像映射后，Adobe Dynamic Media Classic会为其分配一个名称。

   要创建名称，Adobe Dynamic Media Classic会在您正在处理的图像或eCatalog页面的名称中附加一个顺序编号。 您可以输入您选择的名称。
...
1. 要在用户将指针移动到图像映射上时显示变换文本，请在“图像映射”列表中输入文本。在图像映射列表中，选择 **[!UICONTROL 显示]** 菜单和选择 **[!UICONTROL 滚动文本]**. 然后，输入您希望用户在屏幕上看到的文本。 可以在文字处理程序中写入文本，然后将其复制到“变换文本”字段中。

1. 如果您希望在用户将鼠标移动到图像映射上时发生其他操作效果，请定义该操作。在 **[!UICONTROL 显示]** 下拉列表中，选择 **[!UICONTROL 其他操作]**. 输入操作的属性。(转到 **[!UICONTROL 显示]** > **[!UICONTROL 两者兼有]** 为图像映射创建滚动文本和操作。)
...
1. （可选）请执行下列操作之一：
   ...
   * 要删除图像映射或多边形顶点，请在图像上选择一个形状，然后选择 **[!UICONTROL 删除]**. 或者，对于eCatalog，在“订单页面”选项卡上，选择 **[!UICONTROL 清除地图]** 从所有页面中删除图像映射。
...
要更改图像映射的位置、形状和大小，请选择“图像映射”按钮 。然后，选择 **[!UICONTROL 盘]** 工具并按照以下说明操作：

* **更改位置**  — 将指针移动到图像映射的边框附近，但不移到边框上。 当您看见四向箭头图标时，将映射拖到新位置。

* **更改大小和形状**  — 如何更改图像映射的形状和大小取决于您是使用矩形还是多边形图像映射：
...
1. （可选）选择 **[!UICONTROL 生成]** 以根据要创建的文件类型预览生成的数据。 选择 **[!UICONTROL 关闭]** ，以返回“上传元数据”对话框。
...
您可以将图像映射从一个图像或 eCatalog 页面复制到另一个。使用 **[!UICONTROL 复制图像映射]** 以抢先创建它们。 您还可以复制图像映射，以在共享布局或映射结构的图像或页面中重新创建图像映射。
...
您可以定义 URL 模板（也称为 Href 模板），以便在输入图像映射 URL 时更轻松更有效。如果您的大部分图像映射 URL 都共享通用的固定格式，请定义 URL 模板。在输入固定 URL 部分以作为 URL 模板后，每次创建图像映射时，无需输入该部分 URL。您的 URL 模板也可以包含 JavaScript 命令、路径名和参数。默认情况下，URL模板包含一个名为的专有Adobe Dynamic Media Classic JavaScript处理程序 `loadProduct` 会在新窗口中打开图像。
...
默认情况下，URL模板包含一个名为的专有Adobe Dynamic Media Classic JavaScript处理程序 `loadProduct` 以打开一个包含URL目标的新窗口。 但是，您可以使用任何JavaScript代码替换此JavaScript处理程序，或使用以下Adobe Dynamic Media Classic处理程序之一：
...
1. 输入JavaScript代码和完整的URL（变量部分替换为美元符号） [$])。 您可以通过右键单击并选择 **[!UICONTROL 粘贴]**.
1. 选择 **[!UICONTROL 保存]**.
...
* **URL模板选项**  — 选择“URL模板”选项，以将您的URL模板应用到图像或eCatalog页面上的所有图像映射。

* **模板选项**  — 如果您不希望单个图像映射使用URL模板，请在“URL图像映射”列表中取消选择“模板”选项。

## 为图像映射定义其他操作 {#defining-other-actions-for-image-maps}

您可以选择 **[!UICONTROL 显示]** 菜单和选择 **[!UICONTROL 其他操作]** 以触发除滚动文本和网页启动项之外的其他操作。 当用户将指针移动到图像映射上时，您可以启动一个操作。这些操作是万维网联盟 HTML 规范为客户端图像映射定义的属性。它们是：
...
1. 在eCatalog屏幕(eCatalog)的“映射编辑器”屏幕（图像和旋转集）或“映射页面”选项卡上，选择 **[!UICONTROL 显示]** 菜单和选择 **[!UICONTROL 其他操作]**.
1. 使用由万维网联盟 HTML 规范指定的语法，在“图像映射”列表的“其他操作”列中添加支持的属性。
1. 选择 **[!UICONTROL 保存]**.

选择 **[!UICONTROL 显示]** 菜单和选择 **[!UICONTROL 两者兼有]** 您希望图像映射包含滚动文本和操作。
...
在Adobe Acrobat或Adobe InDesign中，创建希望图像映射显示在其中的超链接引用，并指定图像映射的URL位置。 将PDF文件上传到Adobe Dynamic Media Classic时，选择提取链接选项会自动将链接转换为图像映射。

有关更多信息，请参阅Adobe InDesign帮助或Adobe Acrobat帮助。

### 在 Adobe InDesign 中创建图像映射 {#to-create-image-maps-in-adobe-indesign}

1. 在Adobe InDesign，转到 **[!UICONTROL Windows®]** > **[!UICONTROL 交互式]** > **[!UICONTROL 超链接]**.
1. 在“超链接”面板中，选择要制作到图像映射中的文本、框架或图形。
1. 选择 **[!UICONTROL 新建超链接]** 菜单。
1. 在“新建超链接”对话框中，从 **[!UICONTROL 链接到]** 菜单，选择 **[!UICONTROL URL]**.
1. 在URL框中键入或粘贴产品ID。
...
1. 在Adobe Acrobat，转到 **[!UICONTROL 工具]** > **[!UICONTROL 高级编辑]** > **[!UICONTROL 链接工具]**.
1. 拖动以创建图像映射。
1. 在创建链接框中，选择 **[!UICONTROL 自定义链接]**，然后选择 **[!UICONTROL 下一个]**.

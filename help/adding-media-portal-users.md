...
>[!NOTE]
>
>有权更改密码的Media Portal用户可以通过选择 **[!UICONTROL 设置]** > **[!UICONTROL 个人设置]** 和更改“个人设置”屏幕上的密码。
...
1. 在“角色”列表中，选择Media Portal角色，然后选择 **[!UICONTROL 下一个]**.
...
添加新 Media Portal 用户、参与者和参与者用户时，您可以发送一封欢迎电子邮件。您可以配置此电子邮件或告知Adobe Dynamic Media Classic不要发送它。

1. 导航到 **[!UICONTROL 设置]** > **[!UICONTROL 应用程序设置]** > **[!UICONTROL 管理设置]** > **[!UICONTROL 用户管理]**.
1. 在“User Administration Setup（用户管理设置）”屏幕中，选择 **[!UICONTROL 电子邮件设置]**.
1. 在“电子邮件设置”对话框中，指定以下任何设置：

   * **[!UICONTROL 发送电子邮件]**  — 如果您不想通过电子邮件通知新用户您已经注册，请取消选择此选项。

   * **[!UICONTROL 默认密码]**  — 为新用户输入临时密码，或将字段留空以使Adobe Dynamic Media Classic生成随机密码。 用户在首次登录时需要更改密码。
...
* **[!UICONTROL 过滤和排序用户列表]**  — 过滤Media Portal用户列表以查找用户。
...
* **[!UICONTROL 创建用户定义的字段]**  — 创建自定义的用户定义元数据字段，以便它们可以帮助您组织Adobe Dynamic Media Classic中的资产。 必要时，也可以激活或停用这些字段。

...
图像映射是 eCatalog 页面上的一个区域，您可以在该区域内滚动鼠标或单击鼠标，以触发各种操作。例如，当将指针移动到图像映射上时，您会看到项目的滚动文本描述。 单击“图像映射”时，将启动其他操作。例如，您可以打开网页，以便查看器可以更多地了解一个项目或购买它，您还可以启动视频来查看使用中的项目。
...
1. 在“图像映射”区域中，绘制一个矩形或多边形（多个边）图像映射：
   ...
   * **多边形映射**  — 选择“多边形图像映射”工具，并在图像周长周围根据需要多次单击。单击时，Dynamic Media Classic会绘制图像映射的边框。
...
1. 可以使查看者在单击“图像映射”时打开新的网页。在“图像映射”列表面板中的“URL”列中输入网页的 URL。
   ...
   要更便于输入URL（Href模板），请单击&#x200B;**[!UICONTROL 编辑]**&#x200B;并输入模板。

请参阅[使用模板输入JavaScript™和URL](creating-image-maps.md#using_a_template_to_enter_javascript_and_urls)。

1. （可选）在“显示”下拉列表中，单击&#x200B;**[!UICONTROL 滚动文本]**，然后输入您希望用户在图像映射上移动指针时在屏幕上看到的文本。
1. （可选）在“显示”下拉列表中，单击&#x200B;**[!UICONTROL 其他操作]**，然后输入属性，以在用户将指针移动到图像映射上时触发模糊或焦点操作。

   请参阅[定义图像映射的其他操作](creating-image-maps.md#defining_other_actions_for_image_maps)。

1. 单击“**[!UICONTROL 保存]**”。
1. （可选）单击&#x200B;**[!UICONTROL 预览]**&#x200B;以查看具有默认eCatalog查看器预设的eCatalog。

要删除图像映射，请在图像映射列表中选择其名称，然后单击&#x200B;**[!UICONTROL Delete]**。 要在页面中临时禁用图像映射而不删除该图像映射，可以在“图像映射”列表面板上取消选择图像映射的“开”选项。
...
客户在图像映射上移动指针时，您还可以选择显示工具提示文本，以便他们知道自己正在点击什么。
...
* **将图像映射复制到其他页面**  — 单击 **[!UICONTROL 将映射复制到]** （确保您位于“映射页面”选项卡中）。在“选择图像”屏幕上，选择要复制图像映射的一个或多个页面，然后单击&#x200B;**[!UICONTROL 选择]**。
...
1. 如果存在滚动键错误，请单击&#x200B;**[!UICONTROL Rollover_Key Error]**&#x200B;列中的错误，以查看电子表格中必须更改哪些内容才能更正错误。 可以选择并复制该消息的文本，并将其粘贴到电子表格中。
1. 单击&#x200B;**[!UICONTROL 预览]**&#x200B;可在eCatalog查看器中检查页面，单击X可关闭“摘要”屏幕并返回“映射页面”屏幕，或单击&#x200B;**[!UICONTROL 关闭]**&#x200B;可返回“浏览”。
...
1. 在“导入元数据”对话框中，单击&#x200B;**[!UICONTROL 浏览]**，然后选择制表符分隔或XML DTD文件。
...
1. 选择包含要复制的图像映射的eCatalog ，然后单击eCatalog的滚动更新&#x200B;**[!UICONTROL 编辑]**&#x200B;按钮。
1. 在顺序页面选项卡上，单击&#x200B;**[!UICONTROL 复制映射]**。
1. 在“选择资产”对话框中，选择要在其中复制图像映射的eCatalog ，然后单击&#x200B;**[!UICONTROL 选择]**。

Dynamic Media Classic如果目标eCatalog（您将图像映射复制到的eCatalog）的页面或图像数量不同，且大小不同，则会显示一条警告消息。 单击&#x200B;**[!UICONTROL 继续]**&#x200B;以复制图像映射（尽管出现警告）。

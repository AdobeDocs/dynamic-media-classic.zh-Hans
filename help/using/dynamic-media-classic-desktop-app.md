...
请参阅的常见问题解答 [新的Adobe Dynamic Media Classic登录功能现已可用。](/help/using/new-ui-2020.md)
...
有关完整的系统要求，请参阅 [Adobe Dynamic Media Classic桌面应用程序的系统要求](/help/using/system-requirements.md).

Adobe Dynamic Media Classic桌面应用程序中的升级通知不会为生成 *次要* 版本发布。 从次要版本中的修复中获益的客户可以升级。
...
* 更新了Adobe Analytics的身份验证机制。 与新集成或必须从Dynamic Media Classic桌面应用程序中更新某些Analytics变量相关。
  ...
  请参阅 [登录到Adobe Analytics](/help/using/log-analytics.md) 以了解更新步骤。
...
* 由于超时而导致登录问题，从而导致出现以下消息： *可以将此用户分配给无权限的组。 请与管理员联系。*
...
1. 根据您下载的安装程序执行以下操作之一。
   ...
   * **macOS**  — 在 **[!UICONTROL 拖放以进行安装]** 对话框，拖动 **[!UICONTROL Adobe Dynamic Media Classic]** 然后把它放到 **[!UICONTROL 应用程序]**.
...
1. 要登录到Adobe Dynamic Media Classic桌面应用程序，请使用在浏览器中登录到Adobe Dynamic Media Classic的相同凭据。
   ...
   对于 **[!UICONTROL 服务器]** 要使用，请参阅以下生产环境映射：
...
1. 在GUI安装程序窗口中，按照安装步骤进行操作，以便在中记录交互/输入（如安装位置） `Setup.iss` 文件。
...
1. 在Adobe Dynamic Media Classic桌面应用程序右上角附近，点击 **[!UICONTROL 设置]** > **[!UICONTROL 个人设置]**.
1. 在 **[!UICONTROL 个人设置]** 页面，在 **[!UICONTROL 桌面]** 标题，执行以下任一操作：
   * 要从计算机中删除所有AdobeDynamic Media缓存的图像文件，请点击 **[!UICONTROL 清除图像缓存]**，然后点按 **[!UICONTROL 确定]**.
   * 要从计算机中删除所有Adobe Dynamic Media缓存的资源文件，请点击 **[!UICONTROL 清除资产缓存]**，然后点按 **[!UICONTROL 确定]**.
...
* 此 **[!UICONTROL 服务器]** 更新到Adobe Dynamic Media Classic桌面应用程序20.21.1后，下拉列表为空 — 方案：您安装并登录到Adobe Dynamic Media Classic 20.20.1或20.20.2，然后关闭应用程序。 然后您更新到Adobe Dynamic Media Classic 20.21.1。当您尝试登录时， **[!UICONTROL 服务器]** 中的下拉列表 **[!UICONTROL 登录到您的帐户]** 对话框为空。 要解决此问题，您必须 [手动清除缓存](#clear-cache) （请参阅上述步骤）。
...
![“刷新”图标](/help/using/assets/refresh-icon1.png)
*“刷新”图标*

**_我在桌面应用程序中看不到批次集预设。_**<br>点按**[!UICONTROL 上载>作业选项>批次集预设&#x200B;]**. 确保相关**[!UICONTROL 批次集预设&#x200B;]**已启用。 单击**[!UICONTROL 保存并提交上载&#x200B;]**.

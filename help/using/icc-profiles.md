...
ICC（国际颜色联盟）配置文件是一个文件，用于描述如何正确地将图像文件从一个色彩空间转换到另一个色彩空间。 ICC 配置文件有助于为图像获取正确的颜色。 例如，到正确地显示用于在计算机显示器上打印的图像，可以选择 ICC 配置文件。 该配置文件将图像转换到不同的颜色空间并确保颜色正确地联机显示。
...
* Adobe Dynamic Media Classic整合了所有标准的Adobe Photoshop ICC配置文件，以实现强大的图像转换。
* 自定义ICC配置文件可增加灵活性，以满足高级色彩空间转换需求。
* 在详细信息视图中查看配置文件类和PCS类型等详细信息有助于管理ICC设置。
* 上传ICC配置文件非常简单，可确保跨[!DNL Adobe Dynamic Media Classic]中的文件夹进行访问。


## 上传ICC配置文件 {#uploading-icc-profiles}

使用与上载文件相同的方法上载ICC配置文件。 您可以将ICC配置文件存储在任何Adobe Dynamic Media Classic文件夹中。
...
* **[!UICONTROL 色彩空间类型]**：此值是配置文件的“输入”色彩空间，由ICC定义。 颜色空间类型定义颜色空间的组件的数量和对这些组件的解释。 例如，RGB 是具有三个组件（红色、绿色和蓝色）的颜色空间。 颜色空间类型不定义空间的特定颜色特征（例如，原色的色度）。

* **[!UICONTROL PCS类型]**：此PCS类型是配置文件的“输出”颜色空间 — 配置文件连接空间。 例如，颜色配置文件可以将 RGB 转换成 PCS，然后转化成 CMYK。

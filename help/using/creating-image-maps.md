...
1. 绘制矩形或多边形（多条边）图像映射：
   ...
   * **矩形地图**：选择矩形图像映射工具并在页面上拖动以创建矩形。 若要向矩形地图添加点（从而将其更改为多边形地图），请按Ctrl，然后将插入工具放置在所需位置并选择。

   * **多边形地图**：选择“多边形图像映射”工具，并在要包围的图像区域的周边上选择点。 使用多边形密度滑块改变多边形中的点密度。如果选择其他映射，将记住原始密度。如果在多边形中添加、删除或移动了任何点，则原始密度将丢失。 滑块将重置为其最大值。
...
* **更改位置**：将指针移至图像映射边框附近，而不是上方。 当您看见四向箭头图标时，将映射拖到新位置。
...
* **多边形图像映射**：拖动方形选择手柄。 要创建选区手柄，请选择图像映射的边框并开始拖动。
...
您可以定义 URL 模板（也称为 Href 模板），以便在输入图像映射 URL 时更轻松更有效。如果您的大部分图像映射 URL 都共享通用的固定格式，请定义 URL 模板。在输入固定 URL 部分以作为 URL 模板后，每次创建图像映射时，无需输入该部分 URL。URL模板也可以包含JavaScript命令、路径名和参数。 默认情况下，URL模板包含一个名为的专有Adobe Dynamic Media Classic JavaScript处理程序 `loadProduct` 在新窗口中打开图像。
...
* **模板选项**：如果您不希望单独的图像映射使用URL模板，请取消选择“URL图像映射”列表中的模板选项。
...
1. 在“映射编辑器”屏幕（图像和旋转集）或eCatalog屏幕的“映射页面”选项卡(eCatalogs)上，选择 **[!UICONTROL 显示]** 菜单并选择 **[!UICONTROL 其他操作]**.

...
缩放目标会将查看者引导到图像的特定部分。 除了自由形式缩放之外，查看器还可以选择缩放目标缩略图，并缩放到您希望他们聚焦的图像部分。 缩放目标允许您突出显示图像的特定部分。
...
要匹配您的网页上使用的大小，您可以更改缩放查看器的大小。 您可以通过在“设置”屏幕上更改查看器大小来永久更改此设置（如果您是管理员）。 请参阅[设置缩放查看器预设](setting-zoom-viewer-presets.md#setting_up_zoom_viewer_presets)。
...
1. 选择&#x200B;**[!UICONTROL 选择目标]**（箭头），然后选择您创建的缩放目标，并调整目标的大小和位置。
   ...
   * **调整大小**：若要放大或缩小目标，请将指针移动到缩放目标的角上并拖动。
...
1. 可以视情况在“用户数据”字段中输入用户数据。 此字段可供网站设计专业人员向缩放目标添加信息。
1. 选择&#x200B;**[!UICONTROL 保存]**。

...
“发布设置”页面设置确定默认情况下如何将资产从Adobe Dynamic Media Classic服务器传递到网站或应用程序。 如果未指定设置，则Adobe Dynamic Media Classic服务器会根据“发布设置”页面上的默认设置交付资源。 例如，交付不包含分辨率属性的图像的请求会生成一个图像，其值为“图像服务器”页面上的“默认对象分辨率”设置。

管理员可以更改图像服务器、图像渲染器和晕影页面上的默认设置，以建立用于从服务器交付资产的默认设置。

要打开“发布设置”页面，请转到 **[!UICONTROL 设置]** > **[!UICONTROL 应用程序设置]** > **[!UICONTROL 发布设置]**.

>[!NOTE]
>
>“发布设置”页面可供经验丰富的网站开发人员和程序员使用。 Adobe Dynamic Media Classic假定在这些页面上更改设置的用户熟悉Adobe Dynamic Media Classic、HTTP协议标准和惯例以及基本成像技术。

## 图像服务器 {#image-server}

“图像服务器”页面建立了从图像服务器传送图像的默认设置。 设置位于这五个类别中（有关设置的详细说明，请参阅图像服务器页面本身）。

仅在Adobe Dynamic Media Classic支持人员的协助下更改这些设置。

* **[!UICONTROL 目录管理]**  — 这些设置确定Adobe Dynamic Media Classic和目录如何交互。 与大多数Web服务器不同，Dynamic Media图像服务器URL调用转到清单或目录文件，而不是真正的图像文件。 目录文件（不要与 eCatalog 混淆）中包含发布到图像服务器的所有内容的列表，以及每个图像的路径。如果您有 Digimarc ID，请在“Digimarc 用户信息”部分中输入您的用户信息。
...
* **[!UICONTROL 通用缩略图属性]**  — 这些设置与缩略图图像的默认外观和对齐有关。
...
IS支持任意可打印的ASCII字符串。 此 `locale=` 命令具有全局范围，这意味着它应用于整个请求，包括所有嵌套的IS和IR请求、引用的模板和图像图层。 不支持每个请求应用多个区域设置，如每个图层应用不同的区域设置。但是，允许在嵌套的请求中显式覆盖区域设置。
...
* 支持由需要本地化的目录（包括图像、图像集、晕影、材料和查看器配置记录）管理的所有内容。
* 最大程度地减少对 IPS 数据库和 IS 清单机制的更改。
* 在实施RFC IS-63时，增加了对静态内容（如视频和外观）的支持。
* 默认区域设置是可配置的。
...
| 图像集和媒体集 | 对于某些区域设置（例如当eCatalog不同时），整个图像集可能不同，因为图像集是从通用图像集转换为由查看器处理的特定于区域设置的图像集。 更常见的情况是，通用集中的单个ID可以引用本地化的内容。 例如，除控制面板照片外，设备的大多数照片在所有语言中都可能相同。 IS 会自动转换 ID，因此无需生成区域设置特定的图像集。 |

#### 实施资产本地化

Adobe Dynamic Media Classic和图像服务具有一个允许对图像和静态内容进行本地化的界面。

当没有本地化版本时，图像服务器 URL 如下所示：

`https://server/is/image/company/image`

通过本地化，图像服务器URL会添加 `locale=` 路径参数，如下所示：

`https://server/is/image/company/image?locale=de_DE`

当图像服务器收到http调用时， `locale=` 参数是通过 `localeMap` 字段位于 **[!UICONTROL 设置]** > **[!UICONTROL 应用程序设置]** > **[!UICONTROL 发布设置]** > **[!UICONTROL 图像服务器]** > **[!UICONTROL 本地化支持]** 组。

“区域设置映射”字段包含一个用管道符号 (|) 分隔的条目列表。

每个条目由一个以逗号分隔的值列表组成。第一个值是由传递的搜索值 `locale=` 参数。 其余值是后缀/替换值，然后尝试这些值，直到一个值生成现有图像为止。

具体应用后缀值还是应用替换值，取决于“**[!UICONTROL 设置]**”>“**[!UICONTROL 应用程序设置]**”>“**[!UICONTROL 发布设置]**”>“**[!UICONTROL 图像服务器]**”>“**[!UICONTROL 本地化支持]**”组中的“全局区域设置”。

>[!NOTE]
>
>“全局区域设置”设置只有在通过API进行设置时才可用，不能在Adobe Dynamic Media Classic界面中设置。
...
|--- |--- |--- |
| `https://server/is/image/company/image-main-01?locale=de_DE` | `GlobalLocale=mainlocaleMap -` <br><br/> `de_DE,de,main|fr_FR,fr,main` | 在以上替换示例中，GlobalLocale 被设置为 main。区域设置参数de_DE与 `localeMap`. 找到GlobalLocale子字符串并将其替换为第一个相应的值 `de` 在 `localeMap`： `image-de-01`. 如果在图像服务器上找到，则会将其返回。否则，替换第二个值，最终生成 `image-main-01`。 |

如果 URL 中没有定义区域设置，则图像服务器会采用 DefaultLocale（如果定义）并将其应用到 URL。

如果提供了未知或空的区域设置参数 `locale=`，则 `localeMap` 将扫描空值“开头为”。 为未知区域设置应用默认区域设置很重要。

#### 关于defaultImage

图像服务器会为请求的区域设置逐个试用选项。如果未找到匹配项，则区域设置选项将应用于defaultImage，并返回匹配的版本。 因此，每个区域设置都必须包含一个未本地化的图像选项，或者本地化的defaultImage版本在Adobe Dynamic Media Classic中可用。
...
您可以将未知的区域设置映射到特定 ID 或通用 ID。例如，您可以将未知的区域设置映射到英语ID，如果没有未知的区域设置，则映射到通用ID。
...
您还可以具有专用的locSuffix，例如U，用于未知区域设置，如果没有，则强制使用默认图像 `_U` 存在，如下所示：
...
对于本示例，假定您要支持供西方和中东使用的集合。这两个集合都基于通用图像集合，而且都添加或修改某些图像。然后，会针对特定区域设置进一步优化这两个集合，例如 `m1, m2` 两个中东变种的，以及 `w1, w2,` 和 `w3` 三种西方区域设置，但图像共享用于 `w1` 和 `w3`. 未知区域设置仅映射到通用集合，而且无法访问区域设置特定的图像。该映射如下所示：
...
* 本地化仅限基于 ID 的资源调用，无法用于基于路径的资源调用。因此，在调用具有区域设置的视频时，必须以 company/assetID 的形式调用，而不采用视频的完整路径。不能将rtmp用于本地化，因为该方法仅用于基于路径的视频调用。
* 当 localeMap 处于活动状态时，无法使用包含单个视频的“混合媒体集”，否则对媒体集内容的调用会失败。要解决此问题，您可以将单个视频添加到自适应视频集。 然后，将自适应视频集添加到混合媒体集。
* 某些请求没有本地化，例如针对自适应视频集内容的请求。因此，如果您打算将自适应视频集与本地化一起使用，请将自适应视频集放在混合媒体集中。 然后，使用将集调用到混合媒体查看器中 `locale=` 参数。

## 图像渲染器 {#image-renderer}

“图像渲染器”页面建立了从图像渲染服务器交付图像集的默认设置。 设置在以下五个类别中可用（有关设置的详细说明，请参阅“图像服务器”页面本身）：
...
* **[!UICONTROL 默认材质属性]**  — 这些设置为图像建立默认分辨率和锐化设置。
...
“晕影”页面提供了用于建立晕影默认外观的设置（有关选项的详细说明，请参阅页面本身）。

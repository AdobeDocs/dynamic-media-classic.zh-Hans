...
1. 要创建初始模板，请使用以下方法之一：
   ...
   * **首先选择PSD或图像**  — 在“浏览”面板中，选择想要用于模板的PSD文件，转到 **[!UICONTROL 生成]** > **[!UICONTROL 模板基础知识]**.
...
* **[!UICONTROL 预览]**  — 打开“预览”屏幕并在缩放查看器中显示模板。 可以显示模板在网站或应用程序上向用户所显示的效果。

* **[!UICONTROL 参数摘要]** 打开“参数摘要”屏幕。 可以显示模板中每个图层的名称，以及每个图层上已激活的参数名称。

* **[!UICONTROL 文本编辑器v4.3和文本编辑器v4.2]**  — 您可以选择使用最新且功能最丰富的文本编辑器（文本编辑器v4.3）或之前的文本编辑器（文本编辑器v4.2）。创建模板时，默认情况下会选中文本编辑器v4.3。 编辑较旧的模板时，默认选择 Text Editor v4.2。Text Editor v4.3 目前不支持自动换行，因此，在编辑使用自动换行的旧模板时，请使用 Text Editor v4.2 保持模板的保真度完整。如果您的旧模板不使用自动换行，您可以选择文本编辑器v4.3以利用它提供的许多新功能。 例如，增加边距、减少边距、将文本设置为全部大写和根据文本大小复制。
...
* **[!UICONTROL 图层属性区域]**  — 提供用于更改图层的背景颜色、不透明度、大小和位置以及画布的背景颜色、不透明度和大小的工具。 还可以调整阴影和发光效果。请参阅 [使用图层](#working-with-layers).
...
1. 指定以下任何格式选项，然后选择 **[!UICONTROL 应用]**.
   ...
   * **[!UICONTROL 字体]**  — 在“字体”菜单上选择字体。 如果菜单上未显示所需的字体，您可以将其上传到Adobe Dynamic Media Classic。 请参阅字体。
   ...
   * **[!UICONTROL 对齐方式]**  — 选择“对齐”按钮以在文本图层中对齐文本、居中对齐文本或右对齐文本。
   ...
   * **[!UICONTROL 基线偏移]**  — 键入或选择一个数值，通过该数值，所选字符相对于周围文本的基线上下移动。 手动设置分数字或调整随文图形的位置时，该选项特别有用。
...
1. 指定以下任何格式选项，然后选择 **[!UICONTROL 应用]**.
   ...
   * **[!UICONTROL 行距]**  — 键入或选择一个数字值，通过该数值可调整段落中所有行之间的间距量。

   * **[!UICONTROL 全部缩进]**  — 选择以增加文本缩进量。

   * **[!UICONTROL 删除缩进]**  — 选择减少文本缩进的数量。

   * **[!UICONTROL 首行缩进]**  — 指定文本第一行的缩进量。

   * **[!UICONTROL 段前间距]**  — 指定段落中文本第一行上方要显示的空格数。

   * **[!UICONTROL 段后间距]**  — 指定段落中文本最后一行下方要显示的空格数。
...
1. 在“图层属性”面板中，选择以下任意项：
   ...
   * **[!UICONTROL 收缩文本（仅限文本编辑器v4.2）]**  — 要适应文本框，请选择以缩小文本。

   * **[!UICONTROL 自动换行（仅限文本编辑器v4.2）]**  — 要指定文本是否换行或换行方式，请选择换行选项：

   * **[!UICONTROL 换行]**  — 封装文本以适合水平太小的文本框。

   * **[!UICONTROL 不换行]**  — 当文本框水平太小时不换行，而是剪切文本的一部分。

   * **[!UICONTROL 不间断换行]**  — 封装文本以适合文本框，并且不断字。

   * **[!UICONTROL 位置]**  — 指定文本框在画布上的位置。

   * **[!UICONTROL 内边距]**  — 添加边距或裁切图层矩形。 为“左”、“上”、“下”和“右”指定要添加或删除的像素数。 如果要添加边距或负数进行裁剪，请输入正数。
...
* **更改图层的位置**  — 要更改图层在画布上的位置，请在“图层”列表中选择其名称，然后使用以下方法之一：

* **手动更改位置**  — 将指针移到图层边界附近，但不移到图层边界上，当您看到四向箭头光标时，选择并开始拖动。

* **输入位置偏移测量**  — 在X和Y文本框中输入X和Y偏移测量。 这些值表示锚点的 x、y 偏移量（以像素为单位）。

* **旋转图层**  — “旋转”框列出图层旋转到的角度。 要旋转图层，请在“图层”列表中选择其名称并使用以下技术之一：

* **手动旋转**  — 将光标移动到图层一角附近，但不移动到图层一角上。 出现旋转光标后，拖动图层的角。要以 15 度为增量进行旋转，请在拖动时按住 Shift 键。
...
* **不透明度**  — 拖动“不透明度”滑块使任何图层成为半透明，以便底层图像的部分显示通过。 100%的设置是不透明的；0是透明的。
...
>[!NOTE]
>
>如果底层层不够大，无法显示投影或外发光，则有时无法看到投影或外发光的效果。 如果看不到阴影或发光，请考虑向图层添加“填充”值或重新排序图层。 请参阅[更改图层和画布的大小及位置](creating-template.md#changing_the_size_and_position_of_layers_and_the_canvas)和[将图层重新排序](creating-template.md#reordering_layers)。

### 调整阴影或发光效果 {#adjusting-a-shadow-or-glow-effect}

要调整阴影或发光效果，请先在“图层”列表中选择其名称。然后在“模板”屏幕的“图层属性”区域中更改其设置：

* **[!UICONTROL 颜色]**  — 选择“颜色”按钮并选择颜色色板以更改阴影或发光的颜色。 也可以在框中输入颜色值参数。

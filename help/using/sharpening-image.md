...
| 重新采样模式 | `resMode` | `bilin`，`bicub`，`sharp2`，`trilin`<br><br>`bilin`：选择标准的双线性插值。 最快速的重新取样方法；通常会出现一些锯齿伪像。<br>`bicub`：选择双三次插值。 CPU密集度大于`bilin`，但生成的图像更锐利，出现的锯齿伪像更少。<br><br>`sharp2`：选择修改的Lanczos Windows®函数作为插值算法。 它可以产生比双立方体稍微锐利的结果，而成本却更高CPU。<br><br>`trilin`：选择修改后的三线性插值，如果可用，它同时使用较高分辨率和较低分辨率。 建议仅当存在锯齿问题时使用。由于减少高频数据而缩小 JPEG 大小。 | `resMode=sharp2` |

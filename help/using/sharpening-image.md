...
Adobe Dynamic Media Classic还建议向查看器预设添加锐化，然后使用该预设调用查看器。 这样做可以确保查看者中的图像清晰而美观。
...
* **阈值**  — 确定在应用钝化蒙版滤镜时要忽略的对比度范围。 此选项确定锐化的像素必须与周围区域相差多少，才被滤镜看作边缘像素并被锐化。
...
* **[!UICONTROL 锐化2]**  — 产生的结果比锐利一些 **[!UICONTROL 双三次]**，但图像服务器的CPU成本更高。
...
要设置公司的默认锐化选项，请转到 **[!UICONTROL 设置]** > **[!UICONTROL 应用程序设置]** > **[!UICONTROL 发布设置]** > **[!UICONTROL 图像服务器]**. 如果将默认重新取样模式设置为 **[!UICONTROL 锐化2]**&#x200B;时，它始终会锐化缩减像素取样时的图像。
...
在Adobe Dynamic Media Classic中，选择任意图像，转到详细信息视图(通过双击或按 **[!UICONTROL 详细信息视图]** 按钮)，并选择 **[!UICONTROL 锐化]**. 更改任意参数，然后选择 **[!UICONTROL 保存]**. 此过程会告知图像服务器使用这些锐化参数，而不是您在URL中调用的任何命令，例如锐化修饰符或图像预设。 请确保发布页面以查看更改是否生效。

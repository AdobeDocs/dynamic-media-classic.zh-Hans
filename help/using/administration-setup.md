...
所有Adobe Dynamic Media Classic用户都分配有一个角色，该角色可确定他们对Adobe Dynamic Media Classic中的功能的权限和访问权限。 管理员会为自己负责的公司确定不同角色和职责。
...
**Adobe Dynamic Media Classic用户** 可以访问被分派的公司；不能履行任何管理职责。
...
1. 要向用户分配角色，请选择角色选项。
   ...
   参见 [Adobe Dynamic Media Classic用户角色和权限](administration-setup.md#user_administration).

1. 要将用户添加到公司，请选择公司名称。
1. 如果要将用户添加到组（如果要添加Media Portal用户或参与者），请选择 **[!UICONTROL 下一个]** 并添加用户。
1. 选择 **[!UICONTROL 保存]** 以完成用户设置。

   保存之后，系统将提示您是否要将用户添加到其他公司。选择 **[!UICONTROL 添加]** 如果您要将用户添加到公司。

   所有新用户都会获得随机生成的密码；用户首次登录到Adobe Dynamic Media Classic桌面应用程序时需要更改密码。

   在您添加新用户后，会向他们发送欢迎电子邮件。此电子邮件提供了一个临时密码，并说明了如何登录到Adobe Dynamic Media Classic。
   ...
   添加新Media Portal用户时，您还可以转到 **[!UICONTROL 设置]** > **[!UICONTROL 应用程序设置]** > **[!UICONTROL 用户管理]**，然后选择 **[!UICONTROL 上载用户列表]** 并选择一个.csv文件，该文件包含的用户的数量不超过500个。

### 删除用户 {#deleting-a-user}

您可以从Adobe Dynamic Media Classic中删除用户，方法是使其无效。 无效用户会从系统和所有帐户中被删除。
...
### 激活或取消激活用户 {#activating-or-deactivating-users}
...
* **按用户角色筛选**  — 选择 **[!UICONTROL 按用户角色]** 菜单并选择选项以将列表范围缩小到不同类型的用户或管理员。

* **按字段名称筛选**  — 选择 **[!UICONTROL 启用按字段筛选]**. 然后选择 **[!UICONTROL 按字段名称]** 菜单，选择用于筛选列表的列，然后选择“筛选字符”菜单并选择字母。 列表按您选择的字母在某一列上筛选。 要查看完整列表，请取消选择 **[!UICONTROL 启用按字段筛选]** 选项。

* **筛选出无效用户**  — 取消选择 **[!UICONTROL 包含无效]**. 搜索结果只显示系统中的用户。已从系统和您管理的帐户中删除无效用户。

* **按列标题排序**  — 选择一个标题，以按状态对所有用户进行排序，按名字、姓氏或电子邮件的字母顺序、用户角色或有效/无效状态进行排序。
...
* **Vnt对象请求**  — 图像渲染请求，用于返回包含所请求晕影中指定对象的图像。

* **Vnt信息请求**  — 图像渲染请求，用于返回有关所请求晕影的信息。
...
* **选择报表演示文稿类型**  — 在报表类型菜单中，选择 **[!UICONTROL 摘要]**， **[!UICONTROL 每日]**，或 **[!UICONTROL 每月]** 以汇总形式、按天或按月组织数据。 并非所有报告都提供该选项。

* **指定时段**  — 选择选项以定义报表的时间期，然后选择 **[!UICONTROL 更新]** 定义时间段后：

* **预定义的时间段**  — 在预定义报表菜单中，选择一个选项。 例如，选择“上月”以捕获上月的数据。

* **自定义时间段**  — 在预定义报表菜单中，选择 **[!UICONTROL 自定义]**. 然后选择日期 **[!UICONTROL 开始月份]** (或 **[!UICONTROL 开始日期]**)菜单和“月数”（或“天数”）菜单上的日期。 对于域报告和视频内容报告，您可以为捕获报告信息选择特定的开始和结束日期。
...
* **导出报表数据**  — 在数据视图中，根据需要对数据进行排序和排列。 然后打开 **[!UICONTROL 导出]** 并选择一个格式： **[!UICONTROL 制表符分隔]**， **[!UICONTROL 逗号分隔]**，或 **[!UICONTROL HTML格式]**. 数据会以您选择的格式复制到剪贴板。 现在，您可以将数据粘贴到电子表格或应用程序中。

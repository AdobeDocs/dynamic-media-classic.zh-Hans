...
# 高效地管理元数据{#making-more-efficient-use-of-metadata}
...
仅当在元数据字段中正确输入元数据时，元数据才会在搜索和资产组织中使用。 要正确输入元数据并确保所有必需的元数据字段都包含元数据，您可以创建元数据预设。 *元数据预设*&#x200B;是一组预定义的元数据条目。 用户可以在“详细信息视图”中选择一个元数据预设，使用预定义信息填充元数据字段，而不是手动输入元数据。

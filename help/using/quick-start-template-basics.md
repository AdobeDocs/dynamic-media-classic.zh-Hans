...
模板是动态创建的可寻址分层图像文件，类似于图像编辑应用程序（如Adobe Photoshop）中的分层文件。 与包含图层的静态文件（如 PSD 文件）不同，模板可以包括参数。 通过各种参数，图像的不同方面都可以寻址及自定义。
...
使用“参数”页，您可以将模板的任何方面转换为可寻址参数。 您可以更改要在模板中使用的分层图像或文本值。 参数通过URL字符串传递。 这允许您更改任何参数，以便动态自定义从图像服务器生成的图像。
...
本快速入门旨在帮助您开始使用模板基础知识。

## &#x200B;1. 上载文件

上传模板的PSD文件或图像文件。 除了PSD之外，Adobe Dynamic Media Classic还支持许多图像文件格式，但建议模板使用无损的TIFF和PNG图像，因为它们允许透明。
...
在全局导航栏上，选择&#x200B;**[!UICONTROL 上传]**，将PSD文件或其他图像文件从您的计算机上传到Adobe Dynamic Media Classic中的文件夹。 查看[上载模板文件](uploading-template-files.md#uploading_template_files)。

## &#x200B;2. 创建模板

要从PSD文件创建模板，请在上传文件时选择&#x200B;**[!UICONTROL 创建模板]**。 若要从图像创建模板，请在全局导航栏上，转到&#x200B;**[!UICONTROL 生成]** > **[!UICONTROL 模板基础知识]**。 输入画布的宽度和高度测量值。 在页面的右上角附近，选择&#x200B;**[!UICONTROL Designer]**&#x200B;或&#x200B;**[!UICONTROL 开发人员]**，然后将图像拖到“模板”页面上。 您还可以选择图像&#x200B;**，转到&#x200B;**&#x200B;[!UICONTROL 生成] **&#x200B; > &#x200B;** [!UICONTROL 模板基础知识]**。 “模板”页面提供了以下工具：
...
接下来是将图层属性参数化，从而确定 URL 字符串中包含哪些图层属性。 通过参数，您可以更灵活地使用模板。 图层属性参数化之后，可以动态进行更改。
...
在“浏览”面板中选择您的模板，然后选择&#x200B;**[!UICONTROL 预览]**&#x200B;以打开“模板预览”页面。 选择用于交付模板的图像预设，然后单击&#x200B;**[!UICONTROL 复制URL]**&#x200B;按钮。 从“预览”页面复制URL后，您可以在网站或应用程序中使用它。 请参阅[将模板链接至网页](linking-template-web-page.md#linking_a_template_to_a_web_page)。

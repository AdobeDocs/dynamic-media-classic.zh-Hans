...
* “信息”面板的“数据馈送”面板允许您指定CSV文件。 此文件包含要在“信息面板”变换文本中显示的文本。 您还可以安排更新信息的时间。
...
可以选择三个预设响应模板之一，以在“信息面板”中显示文本。 这些预设响应模板决定信息在“信息面板”中的显示方式：列和行的数量、字体大小、字体以及其他设置。 您可以选择预设的响应模板或创建自己的响应模板。
...
1. 选择“响应模板”：
   ...
   * 要创建您自己的响应模板，请选择&#x200B;**[!UICONTROL 自定义]**。 在“用户模板”框中键入模板 XML 定义。 您可以使用预设模板作为自己的起点。

1. （可选）在默认响应框中，键入当Adobe Dynamic Media Classic在检索图像映射信息时遇到错误时要显示的文本。 例如，如果系统收到公司和eCatalog名称，但没有变换标识符，则会向用户显示此消息。
1. 在“响应 TTL”框中，输入要在缓存数据之前等待的小时数：

   * 如果数据在白天频繁更新，则设置较低的数字。
   * 如果数据相对稳定并且不需要在白天频繁更新，则设置较高的数字。 默认为十小时。
...
使用“映射页面”屏幕，您可以查看eCatalog特定页面上图像映射的“信息面板”文本。

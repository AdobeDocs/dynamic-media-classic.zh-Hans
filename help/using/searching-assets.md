...
>[!NOTE]
>
>如果在左侧看不到“资源库”面板，请选择Adobe Dynamic Media Classic窗口下半侧右侧的三角形箭头以打开资源库。)

## 在“浏览”面板中排序文件 {#sorting-files-in-the-browse-panel}

要对显示在右侧浏览面板中的文件夹内容或搜索结果进行排序，请在全局导航栏上选择 **[!UICONTROL 排序]**，然后选择一个选项。 选项包括 **[!UICONTROL 名称]**， **[!UICONTROL 大小(KB)]**， **[!UICONTROL 类型]**， **[!UICONTROL 创建日期]**、和 **[!UICONTROL 上次修改时间]**.
...
* **按资源类型筛选**  — 通过在菜单上选择资源类型，将搜索范围缩小到一种资源类型。

* **文件和文件夹**  — 选择要搜索的位置： **[!UICONTROL 在所有文件和文件夹中]**， **[!UICONTROL 在选定的文件夹中]**，或 **[!UICONTROL 在选定的文件夹和子文件夹中]**.

* **所有发布状态**  — 搜索标记为准备发布、未标记为准备发布或所有文件的文件。

* **条件**  — 如果指定用于搜索的元数据标准，请选择搜索是必须匹配所有条件（ALL搜索）还是任何条件（OR搜索）。

* **搜索条件**  — 创建一个或多个搜索字段以搜索元数据。 要创建检索字段：

   1. 在“高级搜索”中的 **[!UICONTROL 搜索条件]** 标题，并在 **[!UICONTROL 添加字段]** 菜单)，选择向下三角形箭头图标以打开下拉列表。 选择一个元数据视图。 您可以选择 **[!UICONTROL 具有值的所有属性]**， **[!UICONTROL 紧凑视图]**， **[!UICONTROL IPTC]**， **[!UICONTROL 元数据服务器发布字段]**，或 **[!UICONTROL XMP]**.
...
1. 选择 **[!UICONTROL 字段]** 然后，在面板上按照以下说明操作，以便您可以显示筛选菜单并构建筛选操作：
...
1. 在每个面板中，选择一个要作为过滤条件的元数据值。不能在每个面板中选择多个元数据值。
   ...
   >[!NOTE]
   >
   >要从筛选器操作中临时删除字段，请选择 **[!UICONTROL 取消全选]**. 此选项位于每个面板的顶部，在元数据值上方。
...
* **从选择预设菜单删除滤镜操作**  — 运行筛选操作。 然后，转到 **[!UICONTROL 选择预设]** > **[!UICONTROL 删除预设]** 在菜单上。

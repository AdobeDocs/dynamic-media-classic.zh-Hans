...
例如，假定要使用不同颜色的标记显示大写字母的图像，标记为红色、绿色和蓝色。在这种情况下，需要同一大写字母的三种快照。一个红色标记快照，一个绿色标记快照，以及一个蓝色标记快照。还需要红色、绿色和蓝色样本。颜色样本用作用户在样本集查看器中选择的缩略图，以查看红色计费、绿色计费或蓝色计费的次数上限。
...
1. 选择 **[!UICONTROL 保存]**，选择用于存储色板集的文件夹，输入色板集的名称，然后选择 **[!UICONTROL 提交]**.
...
| 是否已发布集？ | **[!UICONTROL 发布晚于]** 保存编辑之前是否选择了保存选项？ | 保存后的集状态 | 保存后的集成员状态 |
|--- |--- |--- |--- |
| 是 | 是 | 已发布 | 已发布 |
...
1. 在“网格视图”中，浏览到SwatchSet，然后选择图像下方 **[!UICONTROL 编辑]**.
1. 执行任何以下操作：

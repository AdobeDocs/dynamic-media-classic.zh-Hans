...
* **[!UICONTROL 已发布的服务器名称]**：此服务器是在所有特定于您帐户的系统生成URL调用中使用的实时CDN （内容分发网络）服务器。 仅当Adobe Dynamic Media Classic支持代表指示您更改此服务器名称时，才会进行更改。

* **[!UICONTROL 原始服务器名称]**：此服务器仅用于质量保证测试。 仅当Adobe Dynamic Media Classic支持代表指示您更改此服务器名称时，才会进行更改。
...
* **[!UICONTROL 显示生成的内容]**：在文件夹中，它显示从资源生成的内容。 例如，在上传时栅格化PDF文件时，Adobe Dynamic Media Classic会为原始PDF中的每个页面创建一个图像。 如果选择“显示生成的内容” ，则在上传原始PDF时，每个生成的图像都会显示。 它与PDF一起显示在PDF上传到的文件夹中。
...
* **[!UICONTROL 显示“刷新子文件夹”按钮]**：打开或关闭显示“子文件夹”的“刷新”按钮。
...
* **[!UICONTROL 保留发布]**：指定上载到Adobe Dynamic Media Classic的替换图像是否保留其正在替换的图像的“准备发布”设置。 或者，在上传时指定设置。
...
* **[!UICONTROL 默认图像映射模板]**：为图像映射中的HREF模板定义默认JavaScript。 要在选择图像映射时运行自定义代码，请在此处设置它。

### 其他设置（到应用程序）

* **[!UICONTROL 垃圾桶清理警告]**：垃圾桶文件夹中的Assets会在七天内自动删除。 选择“在自动删除垃圾桶文件夹中的项目之前发送电子邮件”。 此操作向公司管理员发送通知。 当计划四天内永久删除垃圾桶文件夹中的资源时，会发生这种情况。 请参阅[管理垃圾桶文件夹](/help/using/trash-folder.md)。
...
如果上传的图像以某种方式发生了更改（图像已更改），但对该图像的引用保持不变，请使用此选项。 在上载和翻录® PDF时，覆盖也很有用。 您可以微调Adobe Dynamic Media Classic *`rips`*&#x200B;图像的方式。 您还可以在“上载”对话框中调整ICC颜色配置文件选项，并使用覆盖功能重新上载。
...
“添加预设”和“编辑预设”屏幕提供了以下用于创建和编辑图像预设的选项：
...
* **[!UICONTROL 格式]**：从菜单中选择格式。 选择GIF、JPEG、PDF或TIFF格式会提供更多选项：
  ...
  * GIF 颜色量化选项
    ...
    * **[!UICONTROL 类型]**：选择自适应（默认）、Web或Mac。 如果选择&#x200B;**[!UICONTROL GIF与Alpha]**，则Mac选项不可用。
...
* **[!UICONTROL 锐化]**：选择&#x200B;**[!UICONTROL `Enable Simple Sharpening`]**&#x200B;选项，以在执行所有缩放操作后将基本锐化滤镜应用于图像。 锐化可以帮助补偿在以不同尺寸显示图像时可能导致的模糊。
...
* **[!UICONTROL 两次立方]**：增加图像服务器上的CPU使用量，但生成较锐利的图像，出现的锯齿伪像较少。

* **[!UICONTROL `Sharp 2`]**：可以生成比两次立方选项更锐利的结果，但图像服务器上的CPU成本更高。

* **[!UICONTROL 三线性]**：同时使用较高分辨率和较低分辨率（如果可用）；仅当出现锯齿时才建议使用。 由于减少了高频数据，此方法会减小 JPEG 的大小。

* **[!UICONTROL USM锐化]**：选择以下选项以微调锐化：

* **[!UICONTROL 数量]**：控制应用于边缘像素的对比度。 默认值为1.0。 对于高分辨率图像，最高可将其增加到5.0。 将“量”视为滤镜强度的度量。

* **[!UICONTROL 半径]**：确定边缘像素周围影响锐化的像素数。 对于高分辨率图像，请输入 1 到 2 之间的值。 较低的数值仅锐化边缘像素，较高的数值则锐化范围更宽的像素。 值正确与否取决于图像的大小。

* **[!UICONTROL 阈值]**：要在应用钝化蒙版滤镜时忽略对比度范围，请使用此设置。 也就是说，它可以帮助解决锐化的像素与周围区域必须有多大的不同，才会被视为边缘像素并进行锐化。 为避免引入噪音，请尝试使用`.02`到`0.2`之间的值。 默认值为6，将锐化图像中的所有像素。

* **[!UICONTROL 色彩空间]**：确定图像使用的空间是创建图像的空间，通常是RGB（原始）还是亮度空间（强度）。

* **[!UICONTROL 颜色]**&#x200B;您可以选择以下选项：
...
* **[!UICONTROL 嵌入配置文件]**：选择此选项可在® Photoshop®中打开此图像时，使用此配置文件。
...
默认情况下，仅激活“自适应视频”编码预设（启用或“打开”）。 如果需要，您可以取消激活它们。 在“上载作业选项”对话框的 eVideo 部分中，非活动的自适应视频预设不显示为可选择的选项。
...
1. 要从“上载作业选项”对话框的“eVideo选项”列表中删除预设，请取消选中“自适应视频预设”页面上预设名称旁边的复选框。
1. 选择&#x200B;**[!UICONTROL 关闭]**。
...
>[!NOTE]
>
>除了Adobe Dynamic Media Classic默认启用的“自适应视频”之外，Adobe Dynamic Media Classic管理员可以确定哪些单编码视频编码预设在“上传作业选项”对话框中可见。 Adobe Dynamic Media Classic管理员可以确定哪些视频编码预设在“上载作业选项”对话框中可见。
...
* 从以下自适应视频编码或单编码预设中进行选择：
  ...
  * **[!UICONTROL 自适应视频]**：单个编码预设，可与任何纵横比配合使用，以创建视频以交付到移动设备、平板电脑和桌面。 使用此预设编码的上传源视频具有固定高度。 但是，宽度会自动缩放以保留视频的长宽比。
...
*查看器预设*&#x200B;是一组设置，决定了用户如何在其计算机屏幕和移动设备上查看富媒体资源。 作为管理员，您可以创建查看器预设。 提供适用于一系列查看器配置选项的设置。 例如，可以更改查看器的显示大小、缩放行为和外观。
...
* **[!UICONTROL 混合媒体集查看器]**：在一个查看器中显示不同类型的媒体。 您可以包括样本集、旋转集、图像和视频。 要包含不同类型的内容，请设置选项卡，例如图像集的选项卡和视频的选项卡。 从混合媒体集播放的视频使用标准视频查看器，该查看器具有时间轴和视频控件，例如“停止”、“暂停”、“倒带”和“播放”。 设置混合媒体集查看器预设时，您可以指定要将哪些查看器用于混合媒体集中的不同类型资产。 您还可以使用“网格查看器”或“轮盘式查看器”来查看混合媒体集。
...
* **[!UICONTROL 缩放查看器：弹出]**：在原始图像旁边显示缩放区域的第二个图像。 没有控件；用户将选定内容移动到要查看的区域上方。
...
例如，如果您的主图像大小是 350 X 350 像素，缩放系数为 3，则生成的弹出图像是 1050 X 1050 像素。 如果您的主图像大小是 300 X 300 像素，缩放系数为 4，则弹出图像是 1200 X 1200 像素。 根据JPEG质量设置（建议的设置介于80和90之间），您可以减小文件大小。 建议的缩放系数为 2.5 至 4，取决于您的主图像的大小。
...
* 您可以在以下位置找到支持此视频格式的BlackBerry®设备： [BlackBerry®上支持的视频格式](https://www.blackberry.com/en/secure-communications/support/developers)
...
* **筛选查看器预设列表**：使用这些工具筛选列表：
  ...
  * 打开&#x200B;**查看器**&#x200B;下拉列表并选择一个选项，以便仅查看特定类型的查看器。 选择&#x200B;**[!UICONTROL 所有查看器]**&#x200B;以查看所有查看器。
...
除了通过在用户界面中使用&#x200B;**[!UICONTROL 添加]**&#x200B;来添加查看器预设外，您还可以使用&#x200B;**[!UICONTROL 导出]**&#x200B;来添加查看器预设。 您可以导出现有的HTML5查看器预设，然后将其用作新预设的基础。
...
要创建HTML5查看器预设，请导出现有的HTML5查看器预设以用作基础。 此导出选项非常有用，因为您不必从头创建查看器。 相反，您可以导出与所需内容类似的预设，然后将其作为进行设计调整的基础。

Adobe Dynamic Media Classic中的所有默认标准查看器预设CSS文件都使用指向`Scene7SharedAssets`上资源的相对图像服务路径。 例如，下面是上的查看器预设CSS文件中图像资源的相对路径

`Scene7SharedAsset`: `.s7videoviewer .s7fullscreenbutton[state][selected] { background-image: url(/is/image/Scene7SharedAssets/FullScreenButton_dark_sprite?scl=1&fmt=png-alpha); }`

但是，如果您将查看器CSS文件托管在您自己的网站上，则必须通过在您自己的环境中使用指向图像服务器的显式路径来解析这些相对图像路径。 例如，如果您将上面的相对路径更新为显式路径，则它如下所示，其中`https://s7d1.scene7.com`是图像服务器的直接路径： `https://s7d1.scene7.com/is/image/Scene7SharedAssets/FullScreenButton_dark_sprite?scl=1&fmt=png-alpha`
...
### 复制查看器预设的内嵌代码 {#copying-the-embed-code-of-a-viewer-preset}

使用嵌入的代码功能，可查看所选查看器预设的查看器代码。 您还可以将代码复制到剪贴板，以便将其粘贴到网页中用于部署查看器。
...
元数据预设为管理员提供了一种控制和规范分配给资产的元数据的方式。 在详细信息视图中，用户可以在为此目的而提供的字段中输入有关资源的元数据。 例如，用户可以输入所有者名称、版权说明和地址。 为确保用户准确输入此信息，您可以创建元数据预设。 在详细信息视图中选择元数据预设会使用预定义值填充元数据字段。 例如，会自动输入所有者名称、版权说明和地址。
...
1. 展开要包含在预设中的标题。 在要包含在预设中的不同字段中输入值。
1. 选择&#x200B;**[!UICONTROL 保存]**。
...
Media Portal管理员或公司管理员可以创建自定义的用户定义元数据字段。 自定义字段可帮助您整理Adobe Dynamic Media Classic中的资源。 您可以根据需要将字段标记为活动。 激活后，这些自定义元数据字段名称将显示在详细信息视图元数据面板中。 用户可以在用户定义的元数据字段中输入信息以描述资源。 用户也可以使用户定义的元数据字段成为搜索条件。
...
>[!NOTE]
>
>要使用户定义的字段显示在详细信息视图元数据面板中，请在元数据视图中包含用户定义的字段。 在元数据视图屏幕上，选择&#x200B;**[!UICONTROL `Include UDF (user-defined fields)`]**&#x200B;选项。 有关更多信息，请参阅[元数据视图](application-setup.md#metadata_views)。
...
1. 在“自定义字段”对话框中，设置所需的选项。
   ...
   * **[!UICONTROL 默认值]**：可选。 输入用户在字段中最常输入的值。 您输入的值会成为您所创建的字段的默认值。

   * **[!UICONTROL 应用于]**：可选。 如果希望元数据字段仅应用于特定资源类型，请选择资源类型。
...
只有Media Portal管理员或公司管理员可以管理用户定义的字段。
...
公司管理员首先为要分组到集中的资产定义命名约定。 要引用这些图像，请创建批次集预设。 每个预设都是一组名称唯一、自含的指令。 它定义如何使用与预设方法中定义的命名惯例匹配的图像来构建集合。
...
公司管理员会创建一个默认命名约定，该约定用于任何批集预设方法。 在批量集预设定义中选择的默认命名惯例就是贵公司批量生成所有网站集所需的全部惯例。 创建`Batch Set Preset`以使用您定义的默认命名约定。 如果存在公司定义的默认命名例外情况，则可以为特定内容集创建所需数量的具有替代、自定义命名惯例的批次集预设。
...
有两个元素可用于定义，即“匹配”和“基本名称”。 这些字段是为命名约定定义的所有元素。 它们有助于标识用于命名集合的惯例部分。 此集包含这些变量。 公司的单个命名惯例对其中每个元素使用一行或多行定义。 您可以为唯一的定义使用任意数量的行，并将它们分组为不同的元素，如图像、颜色和视图元素。
...
使用批次集预设可创建、编辑和管理批次集预设。 要涵盖所有必需的资产引入作业，请根据需要创建尽可能多的预设。 有两种形式的批次集预设定义：一种是您设置的默认命名约定，另一种是您立即创建的自定义命名约定。

您可以使用表单字段方法定义批次集预设，也可以使用代码方法定义批次集预设，该方法允许您使用正则表达式。 与&#x200B;**[!UICONTROL 默认命名]**&#x200B;中一样，您可以在表单视图中定义的同时选择&#x200B;**[!UICONTROL 代码视图]**，并使用正则表达式来构建定义。 或者，您可以取消选中任一视图，只使用其一个视图。
...
您可以使用批次集类型&#x200B;**多轴旋转集**&#x200B;创建可自动生成2D旋转集的“方法”。 图像分组使用Row和Column正则表达式来对齐多维数组中的图像资产。
...
根据此信息，系统将按如下方式创建批集类型方法：

![批次集方法图像](assets/se_batch_set_recipe.png)

旋转集的共享资源名称部分的分组将添加到“匹配”字段（高亮显示）。 资产名称中包含行和列的变量部分将分别添加到行和列字段。

上传和发布旋转集后，激活上传作业选项对话框中&#x200B;**[!UICONTROL 批次集预设]**&#x200B;下方2D旋转集方法的名称。

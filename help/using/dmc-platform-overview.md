...
Adobe Dynamic Media Classic与其他系统不同，因为您可以使用Adobe Dynamic Media Classic从单个主要资源和URL请求动态交付媒体。
...
Adobe Dynamic Media Classic动态生成的图像适用于缓存；通常，这些图像是具有唯一的URL调用以标识它们的JPEG图像。 图像在内容交付网络(CDN)上交付，即在Internet上连接以更快地交付内容的服务器系统。 映像从位于全局的服务器分发，然后分发到计算机。 要使用任何CDN供应商实施缓存机制，请将服务器名称更改为指向启用CDN的Dynamic Media图像服务器。 所有Adobe Dynamic Media Classic版本都包含捆绑的CDN缓存。

...
>
> Snapshot是一个可视化演示工具，旨在说明Dynamic Media用于优化和动态图像投放的功能。 尝试使用测试图像或Dynamic Media URL，以便直观地观察各种Dynamic Media图像修饰符的输出，并优化以下各项的智能成像：
>
>* 文件大小（使用WebP和AVIF交付）
>* 网络带宽
>* DPR（设备像素比率）
>
>要了解如何使用快照，请查看[快照培训视频](https://experienceleague.adobe.com/zh-hans/docs/experience-manager-learn/assets/dynamic-media/images/dynamic-media-snapshot)（3分17秒）。
...
* JPG 压缩比率为 10:1，通常生成较小的图像文件。 PNG压缩的比例约为2:1，除非图像包含透明背景。 通常，PNG文件大小大于JPG文件。
...
* 要调整图像大小，请使用`&wid=<value>`和`&hei=<value>`。 这些参数会根据纵横比自动设置图像宽度。
* `&resMode=<value>`控制用于缩减像素采样的算法。 从`&resMode=sharp2`开始。 该值提供了最佳的图像质量。 使用缩减像素取样值`bilin`的速度较快，但通常会导致出现锯齿伪像。
...
图像锐化是控制网站上图像的最复杂方面，其中会发生许多错误。 要详细了解锐化和USM在Adobe Dynamic Media Classic中的工作方式，请参阅以下有用资源：

PDF中名为[锐化Adobe Dynamic Media Classic和图像服务器中的图像的最佳实践白皮书](/help/using/assets/s7_sharpening_images.pdf)。

<!-- Give a 404 See also [Sharpening an image with unsharp mask](https://helpx.adobe.com/photoshop/atv/cs6-tutorials/sharpening-an-image-with-unsharp-mask.html). -->

在Adobe Dynamic Media Classic中，您可以在摄取和/或交付期间锐化图像。 但是，通常使用一种方法锐化图像，但不是同时使用这两种方法。 通过URL在投放时锐化图像通常可提供最佳结果。

可以使用两种图像锐化方法：

* 简单锐化(`&op_sharpen`)：与Adobe Photoshop中使用的锐化滤镜类似，简单锐化会在动态调整大小后对图像的最终视图应用基本锐化。 不过，用户无法配置这种方法。 除非需要，否则最佳做法是避免使用`&op_sharpen`。
* USM锐化( `&op_USM`)：USM锐化是用于锐化的行业标准滤镜。 最佳做法是按照以下准则使用 USM 锐化功能锐化图像。 可以通过 USM 锐化控制以下三个参数：
  ...
  * `&op_sharpen=amount,radius,threshold`
    ...
    * `radius` （0-250，围绕锐化对象绘制的“锐化线”的宽度，以像素为单位。）
      ...
      请注意，参数`radius`和`amount`具有反向关系。 通过增加`amount`可以补偿减少`radius`。 `Radius`允许更细的控制，因为较低的值仅锐化边缘像素，而较高的值锐化较宽的像素范围。
...
将 amount 从 1.75 逐渐增加到 4。 如果锐化仍不是所需的结果，请以小数增量增加半径，然后再次将数量从1.75设置为4。 根据需要，重复此步骤。
...
* 若要针对较小的图像文件大小进行优化，但使图像质量保持在可接受的级别，请将`qlt=`值设置为80。 值低于70到75会导致图像质量显着下降。
* 要停留在中间，请将`qlt=`值设置为85作为最佳实践。
* 使用色度标志`qlt=`

  * `qlt=`参数具有第二个设置，通过该设置，您可以使用正常值`,0`打开RGB色度缩减像素采样（默认值），或使用值`,1`关闭它。
  * 在关闭RGB色度缩减像素采样(`,1`)后开始。 此设置通常会获得较好的图像质量，尤其是具有很多清晰边缘和对比度的合成图像。

作为JPG压缩的最佳实践，请使用`&qlt=85,0`。

## JPEG 大小调整的最佳做法 (&amp;jpegSize=) {#best-practices-for-jpeg-sizing-jpegsize}

如果要确保图像不超过特定大小，参数`jpegSize`非常有用。 此参数用于交付给内存有限的设备。

* 此参数设置为千字节(`jpegSize=<size_in_kilobytes>`)。 它定义了图像传送允许的最大大小。
* `&jpegSize=`与JPG压缩参数`&qlt=`交互。 如果具有指定JPG压缩参数(`&qlt=`)的JPG响应未超过`jpegSize`值，则按定义返回带有`&qlt=`的图像。 否则，`&qlt=`将逐渐减小，直到图像符合允许的最大尺寸。 或者，如果系统无法适应图像，则会返回错误。

在将JPG图像传送到内存有限的设备时，最佳做法是设置`&jpegSize=`并包含参数`&qlt=`。
...
如果图像需要进一步优化，请从设置为0.2或0.3的半径开始逐步微调锐化（钝化蒙版）参数。 然后，逐渐将数量从1.75增加到最大值4（相当于[!DNL Adobe Photoshop]中的400%）。 检查是否获得了所需的效果。

如果锐化效果仍不令人满意，请按小数点增加 radius 值。 对于每增加一个小数，将数量重置为1.75，并逐渐将其增加到4。 重复此过程，直到获得所需的效果。 虽然上述值是创意工作室已验证的方法，但请注意，您可以使用其他值并遵循其他过程。 结果是否令您满意是一个主观问题；因此，需要进行结构化的试验。

在实验过程中，以下一般建议有助于优化您的工作流：

* 直接在URL上或使用[!DNL Adobe Dynamic Media Classic]图像调整工具实时测试不同的参数。 后者为调整操作提供实时预览。
* 作为最佳实践，请记住，您可以将Dynamic Media图像服务命令分组到图像预设中。 图像预设是一组具有自定义预设名称（如`$thumb_low$`和`$product_high$`）的URL命令宏。 URL路径中的自定义预设名称将调用这些预设。 此功能帮助您针对网站上的图像的不同使用方式管理命令和质量设置，并缩短 URL 的总体长度。

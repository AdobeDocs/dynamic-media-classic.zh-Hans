...
* psb文件。
* 不支持色彩空间不是CMYK、RGB、灰度或位图的PSD文件。 不支持双色调、Lab和索引色空间。
* 位深度大于16的PSD文件。
* 包含浮点数据的TIFF文件。
* 具有Lab色彩空间的TIFF文件。
...
Adobe Dynamic Media Classic支持多种静态文件格式。 静态内容是“按原样”发布的任何资源。 例如，CSS、PDF和XML。
...
您选择的用于在Adobe Dynamic Media Classic上存储内容的文件夹命名方法和结构取决于贵组织的需求。 以下是一些文件夹结构示例：
...
### FTP安全 {#about-ftp-security}

Dynamic Media Classic为FTP访问强制实施以下安全策略：

| 安全策略 | 说明 |
| --- | --- |
| 用户隔离 | 每个FTP用户被限制到自己的目录，并且无法访问属于其他用户的目录。 |
| 独特帐户 | 每个FTP用户或集成都需要专用FTP帐户。 不允许共享帐户。 请参阅[管理FTP帐户](/help/using/ftp-accounts.md)。 |
| 配置基线 | FTP服务器配置遵循已建立的安全基线。 |
| 正在进行的审计 | FTP服务器应用程序和底层操作系统将定期审核和监控安全漏洞，并根据需要进行更新。 |
| *新*&#x200B;客户的安全FTP | 新客户会配置安全FTP。 *现有*&#x200B;客户仍可访问旧版FTP。 |

## 关于上传文件 {#uploading-your-files}

您可以从桌面上载单个文件或使用FTP上载文件夹。 如果要上载超过100 MB的文件或上载整个文件夹和子文件夹，请选择&#x200B;**通过FTP**&#x200B;选项卡。
...
在上传](image-editing-options-upload.md#image-editing-options-at-upload)上查看[映像微调选项。
...
1. 指定 HTTP 地址。
1. 指定是否只在上传文件后运行。
1. 指示您是要在每次完成此作业时运行此请求，还是只要在发布文件时运行此请求。

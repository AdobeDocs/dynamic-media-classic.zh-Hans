...
1. 采用以下方法之一开始创建 eCatalog：
   ...
   **开始从eCatalog屏** 幕单击“构建”>“eCatalog”。在资源库中选择一个文件夹，将该文件夹中的文件拖到 eCatalog 页面的“排序页面”选项卡中。

   ***注**:要按名称而非缩览图视图资源库中的项目，请在“个人设置”中为“默认资源库”视图选择“名称”选项。*
...
1. 根据需要采用以下方法之一重新排序页面：
   ...
   **拖** 动将页面或跨页拖动到新位置。垂直栏显示要移动的页面的位置。

   **移到按** 钮选择一个页面或页面跨页，单击移到按钮，然后在菜单上选择您希望之前显示页面的页面。

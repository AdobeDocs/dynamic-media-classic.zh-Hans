...
缩放查看器预设可确定缩放查看器的样式、行为和外观。Dynamic Media Classic提供了许多用于自定义查看器和设置查看器外观的选项。 Dynamic Media Classic附带默认的基本（快速）、浮出和自定义缩放查看器预设。 如果您是管理员，则可以创建新的公司缩放查看器预设，或编辑默认预设并使用新名称保存。
...
Dynamic Media Classic提供以下缩放查看器预设：
...
* **缩放查看器：自定**&#x200B;义为图像、具有多个视图的图像集或色板集提供引导式或未引导式缩放。

...
Dynamic Media经典包括100多个图像服务命令，用于调整和优化图像以及渲染结果。 以下准则可以帮助您简化流程，并使用一些基本命令和最佳做法快速获得较好的效果。
...
* 如果URL中未提供任何格式命令，则Dynamic Media图像服务默认为JPG，以便投放。
...
动态减小图像大小是Dynamic Media图像服务执行的最常见的任务之一。 这包括指定大小，以及指定用于减小图像的缩减像素采样模式（可选）。
...
最佳实践白皮 [书《在AdobeDynamic Media经典和图像服务器上锐化图像](/help/assets/s7_sharpening_images.pdf)》。

另请参阅 [使用USM锐化锐化图像](https://helpx.adobe.com/photoshop/atv/cs6-tutorials/sharpening-an-image-with-unsharp-mask.html)。

借助Dynamic Media经典，您可以在摄取、投放或同时在两者中锐化图像。 大多数情况下，只应使用一种方法锐化图像，而不能同时使用两种方法。传送时在 URL 上锐化图像通常可以获得最佳的效果。
...
* Unsharp masking ( `&op_USM`) - Unsharp masking is an industry standard sharpening filter. 最佳做法是按照以下准则使用 USM 锐化功能锐化图像。可以通过 USM 锐化控制以下三个参数：
   ...
   * `&op_sharpen=amount,radius,threshold`
      ...
      * `threshold` （0-255，效果的敏感性。）
      ...
      Dynamic Media经典帮助主题 [有关锐化图像](https://help.adobe.com/en_US/scene7/using/WS389B162D-2981-41e5-9253-15D22D2ECBC8.html)。
...
* 实时尝试和测试不同的参数，可以直接在Dynamic Media经典URL上或使用Dynamic Media经典的图像调整功能，该功能为调整操作提供实时预览。

...
缩放图像必须具有高像素数。需要大量像素，以便查看者在缩放图像时能够清晰地查看图像细节。 在将图像上传到Dynamic Media Classic之前，请确保您使用的图像大小合适。

在全局导航栏上，单击&#x200B;**[!UICONTROL 上传]**&#x200B;开始上传图像。
...
* **色彩空间** - RGB是Web图像展示的色彩空间；CMYK图像在上传时会自动转换为RGB。建议上载嵌有用于转换为 RGB 的 ICC 颜色配置文件的 CMYK 图像。另请参阅 ICC 配置文件。

...
欲了解更多信息，请参见 [即时发布通知PDF](/help/assets/rendering-instant-publish-notification.pdf).
...
要取消发布作业，请转到“作业”页面并选择 **[!UICONTROL 取消]**. 在“作业”页的“已调度”选项卡上，通过取消选中或选中作业的“活动”列中的复选框，可以暂停或恢复作业。
...
您可以手动发布各个资源，而不是创建发布作业。当您发布图像集或自适应视频集等集时，集（或“父级”）以及该集中的所有成员（或“子级”）都会被发布。
...
您可以手动取消发布各个资源。取消发布样本集或eCatalog等集时，集（或“父项”）本身将进入未发布状态。 但是，该集内的成员（或“子项”）不受影响；相反，它们各自保留其现有的已发布或未发布状态。

已发布的资产在用户界面中由资产名称左侧的中心（已发布状态）带有白色复选标记的绿色圆形图标表示。 取消发布资产后，图标会变为灰色，并在其中插入一个斜杠（未发布状态）。

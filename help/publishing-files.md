...
创建发布作业以发布已上传到Dynamic Media Classic服务器的资产，但选择不自动发布。 您可以执行一次性发布作业或计划作业以定期重复。 Dynamic Media Classic优惠了高级发布选项，用于发布到特定服务器以及用于重新发布已发布的资产的选项。
...
如果希望稍后发布作业，请在“发布”页面中，选择&#x200B;**[!UICONTROL 一次]**，然后单击&#x200B;**[!UICONTROL 稍后计划]**&#x200B;下拉列表。 使用日历和时间滑块选择运行发布作业的日期和时间。

### 创建重复发布作业 {#creating-a-recurring-publish-job}

在“发布”页面上选择&#x200B;**[!UICONTROL 重复]**，创建循环发布作业。

然后，选择&#x200B;**[!UICONTROL Daily]**、**[!UICONTROL Weekly]**、**[!UICONTROL Monthly]**&#x200B;或&#x200B;**[!UICONTROL Custom]**&#x200B;的“重复”选项，然后指定要重复发布作业的时间。 Dynamic Media Classic提供用于计划重复发布作业的日历工具。 您可以单击&#x200B;**[!UICONTROL 自定义]**&#x200B;选项，并在规则文本字段中输入规则以描述自定义作业间隔。
...
* **发布到**  — 要仅将资产发布到特定服务器，请选择一种服务器类型。

* **发布**  — 默认情况下，Dynamic Media经典仅发布之前尚未发布的新资产（自上次发布后新建）。但是，您可以单击&#x200B;**[!UICONTROL 完整发布]**&#x200B;来发布自上次发布以来已更新或更改的资产。 如果要发布eCatalog并且希望读者能够按关键字搜索它，请选择&#x200B;**[!UICONTROL 完全包含搜索数据]**。
...
要取消发布作业，请转到“作业”页，然后单击&#x200B;**[!UICONTROL 取消]**。 在“作业”页的“已计划”选项卡上，您可以选中或取消选中作业的“活动”列中的对应复选框以暂停或恢复作业。
...
未发布的资产在用户界面的资产名称左侧会显示一个灰色的圆形图标，并在其中加上斜杠（未发布状态）。 在发布资源后，该图标将变为绿色，并且中心带有白色复选标记（已发布状态）。
...
1. 执行以下操作之一：
   ...
   * 在“网格”视图、“列表”视图或“详细信息”视图中，单击资产名称左侧带斜杠的灰色圆形图标。
...
在用户界面中，发布的资源以资源名称左侧中心带有白色复选标记的绿色圆形图标表示（已发布状态）。取消发布资产后，图标会变为灰色，并在资产中穿过斜杠（未发布状态），

**手动取消发布资源:**

1. 执行以下任一操作：

   * 在“网格”视图、“列表”视图或“详细信息”视图中，选择一个或多个已发布的资产。
...
Dynamic Media Classic资源分发在内容投放网络(CDN)上。 CDN 是指联成网络的计算机服务器系统，这些服务器以完全透明的方式合作，将内容（尤其是大型媒体内容）传递给最终用户。在 CDN 系统中，Web 内容存储在整个 Internet 的网络缓存中（称为边缘缓存网络）。Web内容从Web缓存交付给最终用户，以加快投放。
...
新发布的Dynamic Media Classic内容可立即提供给最终用户，并快速填充边缘缓存网络。 但重新发布的内容（与之前发布到图像服务器的图像完全同名的图像）在 CDN 上最多需要十个小时才会更新。最终用户看到的则是 CDN 网络上的网络缓存中的内容。因此，您的Dynamic Media经典重新发布的资产在十小时内不会显示给最终用户。

如果希望重新发布的图像资源在可用前的延迟时间小于 10 小时，可以刷新 CDN 上的网络缓存。刷新这些网络缓存可以从 CDN 网络缓存中删除旧内容，并将其替换为最新发布的资源。

要刷新缓存，请在全局导航栏上单击&#x200B;**[!UICONTROL 文件]** > **[!UICONTROL 使CDN]**&#x200B;失效。 将从缓存中删除所有选定的文件。如果没有可发布资源，或者您不是公司管理员，则“从 CDN 删除”选项不可用。

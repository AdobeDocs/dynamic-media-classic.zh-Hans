...
* **MP4** - Dynamic Media Classic建议将MP4作为首选视频文件格式。将 MP4 文件用于以下内容：
   ...
   * 桌面的 HTTP 动态流。
   * HTTP 实时流（Apple 的流协议）。
   * 渐进式视频投放到Android™、BlackBerry®和Windows®移动设备
...
1. 在“上载”页中，单击“从桌面&#x200B;]**”选项卡。**[!UICONTROL 
1. 在“上载”页面的&#x200B;**[!UICONTROL “选择要上载的文件]**”面板中，单击&#x200B;**[!UICONTROL 浏览]**，导览至MP4视频文件，然后单击&#x200B;**[!UICONTROL 打开]**。
1. 在&#x200B;**[!UICONTROL 选择文件夹目标]**&#x200B;面板中，为已上载的文件选择文件夹。
1. 在“上传”页面上，确保选中&#x200B;**[!UICONTROL 上载后发布]**。
1. 单击“**[!UICONTROL 提交上载]**”。
...
1. 在全局导航栏上，单击“**[!UICONTROL 上载]**”。
1. 在“上载”页中，单击“从桌面&#x200B;]**”选项卡。**[!UICONTROL 
1. 在&#x200B;**[!UICONTROL 选择要上载的文件]**&#x200B;面板中，单击&#x200B;**[!UICONTROL 浏览]**，导航到主控源视频文件，然后单击&#x200B;**[!UICONTROL 打开]**。
1. 在&#x200B;**[!UICONTROL 选择文件夹目标]**&#x200B;面板中，为已上载的文件选择文件夹。
1. 在页面的右下角，单击“**[!UICONTROL 作业选项]**”。
1. 在“上载作业选项”对话框中，展开&#x200B;**[!UICONTROL EVideo选项]**，然后执行下列操作之一：
   ...
   * 可选。如果要使用单个编码设置，请展开&#x200B;**[!UICONTROL 单个编码预设]**，然后选择您想要用于桌面、移动设备和平板电脑的编码选项。
...
1. 在“上传”页面上，确保选中&#x200B;**[!UICONTROL 上载后发布]**。
1. 在“上载”页面的右下角，单击“**[!UICONTROL 提交上载]**”。

*如果要重新编码以前上载的视频文件*

1. 在Dynamic Media Classic的“浏览”面板中，导航到视频并将其选中。
1. 单击“**[!UICONTROL 文件]**”>“**[!UICONTROL 重新处理]**”。
1. 在“重新处理资产”对话框中，展开&#x200B;**[!UICONTROL EVideo选项]**，然后执行以下操作之一：
...
* **视频文件格式**  — 与ZIP文件类似，视频文件格式决定了视频文件中包含文件的方式。视频文件通常包含多个轨道 — 一个视频轨道（没有音频）和一个或多个音频轨道（没有视频） — 这些轨道相互联系并且同步。视频文件格式决定了这些不同的数据轨道和元数据的组织方式。

* **视频编解码器**  — 视频编解码器描述了对视频进行编码的算法。视频播放器根据其编码解码器对视频进行解码，然后在屏幕显示一系列图像或帧。编解码器将视频文件存储为播放视频所需的信息量降至最低。只会存储一个帧和下一个帧之间差异的相关信息，而不存储每个单独帧的相关信息。由于大多数视频在一个帧之间变化不大，因此编解码器允许高压缩率，这会导致较小的文件大小。
...
下表介绍了在对源视频文件进行编码时必须具有的推荐大小、宽高比和最低比特率：
...
1. 安装后，右键单击视频文件（仅限Windows®）并选择“MediaInfo”，或打开MediaInfo并将视频文件拖动到应用程序中。 您会看到与视频文件相关的所有元数据，其中包括其宽度、高度和 fps。
...
例如，宽度为1440 x 1080的视频的宽高比为1440/1080，即1.33。在这种情况下，您需要选择宽高比为4:3的视频编码预设来对视频文件进行编码。
...
当您选择目标编码预设时，请记住要考虑视频最终用户的连接速度。 选择数据速率为该速度 80% 的预设。例如，如果目标最终用户的连接速度为 1000 Kbps，最佳预设是视频数据速率为 800 Kbps 的预设。
...
>[!NOTE]
>
>通常，您使用的数据率越高，视频的显示效果越好，您使用的分辨率越高，您必须保持观看质量的数据率就越高（与较低分辨率相比）。

因为分辨率和数据速率是关联的，在编码视频时您有两个选择：

* 选择一个数据率，然后以最高分辨率进行编码，该最高分辨率以您选择的数据率显示最佳。
...
“视频预设”页和“自适应视频预设”页由一个表组成，该表格列表了每个视频预设的活动状态、预设名称、预期播放设备、视频大小和数据速率。
...
1. 在Dynamic Media Classic中，在全局导航栏上，单击&#x200B;**[!UICONTROL 设置]** > **[!UICONTROL 应用程序设置]** > **[!UICONTROL 视频预设]** > **[!UICONTROL 自适应视频预设]**&#x200B;或&#x200B;**[!UICONTROL 单个编码预设]**。

   自适应视频预设和单个编码预设的页面包含一个表格，其中列表了每个视频预设的“活动”状态、预设名称、预期的播放设备、视频尺寸和数据率。
...
激活的视频预设将显示在“上载作业选项”对话框中。当用户在上传过程中上载视频文件时，将显示该对话框。 他们可以从所有激活的编码预设列表中进行选择。

1. 在Dynamic Media Classic中，在全局导航栏上，单击&#x200B;**[!UICONTROL 设置]** > **[!UICONTROL 应用程序设置]** > **[!UICONTROL 视频预设]**。
1. 执行以下任一操作：
...
1. 在“添加视频预设”或“编辑视频预设”页中，设置所需的视频预设选项。
   ...
   | 说明 | 描述视频预设。当您将指针移到“上载作业选项”对话框中的预设名称上时，用户在其中选择转码选项时，输入的内容将作为工具提示显示。 |
   ...
   | 分辨率大小 | 分辨率大小由像素宽度以像素高度表示，决定大小。 以像素为单位输入宽度和高度值，或拖动滑块来输入这些值。分辨率范围列出了标准分辨率大小。宽度和高度值将自动遵循所选的高宽比。例如，如果您选择 4:3 作为高宽比，并且为宽度输入 400，则会自动为高度输入 300。如果为高宽比设置选择了“自动缩放”，则自动将分辨率大小的宽度值设置为“自动”。单击“预览”，以打开浏览器窗口并查看您在此处的分辨率选择。 |
   | 编码文件后缀 | 输入后缀。该后缀会附加到生成的编码视频文件的后面。您可以在名称中输入连字符和下划线；不允许使用空格和特殊字符。 |
   | 其他设置 | Dynamic Media Classic根据最佳实践编码准则自动确定所有其他编码设置。 |

1. 执行以下任一操作：

   * 如果添加或编辑了“视频预设”，请单击“**[!UICONTROL 保存]**”。
   * 如果通过现有的预设添加视频预设，请单击“**[!UICONTROL 另存为]**”。

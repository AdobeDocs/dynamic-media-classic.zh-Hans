...
缩放目标会将查看者引导到图像的特定部分。除了自由缩放，查看者还可以单击缩放目标缩览图，并缩放到您希望他们关注的图像部分。 通过缩放目标，可突出显示图像的有趣或诱人之处。
...
在“缩放目标编辑器”屏幕上，单击&#x200B;**[!UICONTROL “选择目标]**”按钮（箭头）以在更改目标的大小或位置之前选择。 要在图像上创建缩放目标，请单击&#x200B;**[!UICONTROL 添加目标]**（矩形）。 “缩放目标编辑器”页还优惠了用于删除、复制和命名缩放目标的工具。

### 创建缩放目标 {#creating-a-zoom-target}

要创建缩放目标，请打开“缩放目标编辑器”页面并执行以下操作：
...
1. 单击&#x200B;**[!UICONTROL 选择目标]**（箭头），单击以选择您创建的缩放目标，并调整目标的大小和位置。
   ...
   * **调整大小**  — 将指针移到缩放目标的一角上，然后拖动以放大或缩小目标。
...
要编辑缩放目标，请在“缩放目标编辑器”页面上使用以下方法：

* **重新定位**  — 使用“选择目标”按钮（箭头），单击目标以选择它。然后，将目标拖动到其他位置。

* **调整大小**  — 使用“选择目标”按钮（箭头），单击目标以选择它。要放大或缩小目标，请将指针移到缩放目标的某个角上并拖动。

* **删除**  — 单击屏幕右侧的目标缩览图图像。然后单击&#x200B;**[!UICONTROL 删除目标]**。

* **重命名**  — 单击屏幕右侧的目标缩览图图像。然后在&#x200B;**[!UICONTROL 名称]**&#x200B;文本字段中输入名称，然后单击&#x200B;**[!UICONTROL 保存]**。

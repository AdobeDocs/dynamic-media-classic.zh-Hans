...
要向 Adobe Analytics 通知 Adobe Analytics 报告中所需的信息，请转到“Adobe Analytics 配置”屏幕。配置报告后，此屏幕列表会针对您需要的每个查看器事件显示相应的Adobe Analytics变量和动态媒体经典变量的相关信息。 这些查看器事件-Adobe Analytics变量——动态媒体经典变量组合确定报告的信息。

除了将查看器事件与变量相关联之外，“Adobe Analytics 配置”屏幕还提供了用于激活、编辑和删除查看器事件的工具。

>[!NOTE]
>
>在Adobe Analytics更改Adobe Analytics报告设置时，请确保从AdobeDynamic Media Classic中重新登录到Adobe Analytics，重新保存Adobe Analytics配置设置，然后重新发布。
...
使用“Adobe Analytics配置”屏幕将查看器事件与Adobe Analytics变量和Dynamic Media Classic变量相关联。 对于每个查看器事件，选择一个Adobe Analytics变量和一个Dynamic Media Classic变量。 有关打开“Adobe Analytics 配置”屏幕的说明，请参阅[登录到 Adobe Analytics](log-analytics.md#log_in_to_adobe_analytics)。
...
| PLAY | **X**（视频） | 在视频查看器中，用户单击“播放”开始播放视频时。<br><br>**注意：** 如果您使用基于Adobe Analytics心跳的视频报告，则在Dynamic Media Classic中配置Adobe Analytics时，无需将任何变量映射到此查看器事件。 视频心跳可以与现成的Dynamic Media Classic HTML5视频和混合媒体查看器配合使用。 视频播放器生成跟踪数据以在 Adobe Analytics 视频报告中查看。See [Enabling Adobe Analytics Video Reports](enabling-analytics-video-reports.md). |
| PAUSE | **X**（视频） | 在视频查看器中，用户单击“暂停”以暂停视频时。<br><br>**注意：** 如果您使用基于Adobe Analytics心跳的视频报告，则在Dynamic Media Classic中配置Adobe Analytics时，无需将任何变量映射到此查看器事件。 视频心跳可以与现成的Dynamic Media Classic HTML5视频和混合媒体查看器配合使用。 视频播放器生成跟踪数据以在 Adobe Analytics 视频报告中查看。See [Enabling Adobe Analytics Video Reports](enabling-analytics-video-reports.md). |
| STOP | **X**（视频） | 在视频查看器中，用户单击“停止”会停止播放视频。<br><br>**注意：** 如果您使用基于Adobe Analytics心跳的视频报告，则在Dynamic Media Classic中配置Adobe Analytics时，无需将任何变量映射到此查看器事件。 视频心跳可以与现成的Dynamic Media Classic HTML5视频和混合媒体查看器配合使用。 视频播放器生成跟踪数据以在 Adobe Analytics 视频报告中查看。See [Enabling Adobe Analytics Video Reports](enabling-analytics-video-reports.md). |
| MILESTONE | **X**  (视频) | 在视频查看器中，用户在观看到视频的 0%、25%、50%、75% 或 100% 时会生成里程碑事件。<br><br>**注意：** 如果您使用基于Adobe Analytics心跳的视频报告，则在Dynamic Media Classic中配置Adobe Analytics时，无需将任何变量映射到此查看器事件。 视频心跳可以与现成的Dynamic Media Classic HTML5视频和混合媒体查看器配合使用。 视频播放器生成跟踪数据以在 Adobe Analytics 视频报告中查看。See [Enabling Adobe Analytics Video Reports](enabling-analytics-video-reports.md). |
| 样本 | X （弹出、缩放） | 此查看器事件映射到Dynamic Media Classic中的PAGE查看器事件。 |
...
### 动态媒体经典变量 {#scene-variables}

For each viewer event on Adobe Analytics Configuration screen, choose a Adobe Analytics variable and a *Dynamic Media Classic variable*. 动态媒体经典变量表示您可以获取的报表数据。 例如，`searchTerm` 变量可列出在 eCatalog 搜索中使用的关键字。
...
* **编辑**&#x200B;选择查看器事件，然后单击 **[!UICONTROL 视图/编辑变量]** 灰色按钮。 在动态媒体经典变量和Adobe Analytics变量下拉列表中，从各个列表中选择一个不同的变量。 有关详细信息，请参阅将Adobe Analytics变量分配给Dynamic Media Classic查看器事件和变量。

...
* **颜**
色选择“颜色”选项。然后，从“角”下拉列表中选择颜色最好地表示要裁切的空白颜色的图像角。
...
* **基于颜色修**
剪指定0可仅在像素与您在图像角中选择的颜色完全匹配时裁剪像素。数字越接近 1，允许的色差越大。

* **基于透明度的**
修剪指定0可仅裁剪完全透明的像素；接近1的数字意味着更加透明。
...
| 半径 | 控制效果的半径。<br><br>值范围是0-250。该效果在图像中的所有像素上运行，并从所有方向的所有像素辐射出来。半径以像素为单位。例如，要使 2000 x 2000 像素图像和 500 x 500 像素图像具有类似的锐化效果，请在 2000 x 2000 像素图像上设置 2 个像素的半径值，而在 500 x 500 像素图像上设置 1 个像素的半径值。较大的值用于具有更多像素的图像。 |

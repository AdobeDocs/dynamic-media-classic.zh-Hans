...
在Dynamic Media Classic中，您可以选择ICC用户档案，在上传图像时将图像转换为不同的色彩空间。 默认情况下，所有标准Photoshop ICC用户档案都在Dynamic Media Classic上可用。 要在“上载”屏幕中查看颜色配置文件的名称，请选择“颜色配置文件”菜单。然后选择“自定义从>到”，再从“转换至”和“转换到”菜单中选择 ICC 配置文件的名称。
...
通过与上载文件时所使用的技术相同的技术上载 ICC 配置文件。您可以将ICC用户档案存储在任何Dynamic Media Classic文件夹中。
...
* **色彩空间类** 型 — 此数字是用户档案的&quot;输入&quot;色彩空间，由ICC定义。颜色空间类型定义颜色空间的组件的数量和对这些组件的解释。例如，RGB 是具有三个组件（红色、绿色和蓝色）的颜色空间。颜色空间类型不定义空间的特定颜色特征（例如，原色的色度）。

* **PCS Type**  — 此PCS类型是用户档案的&quot;output&quot;色彩空间，即其用户档案连接空间。例如，颜色配置文件可以将 RGB 转换成 PCS，然后转化成 CMYK。
